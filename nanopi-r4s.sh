...
: "${TERM:=xterm}"
# VPS 优化脚本 - NanoPi R4S (Armbian)
#
# 用法:
#   sudo bash nanopi-r4s.sh                    # 完整安装（底层优化 + 软件依赖）
#   sudo bash nanopi-r4s.sh --optimize-only    # 仅底层优化
#   sudo bash nanopi-r4s.sh --uninstall        # 卸载所有优化
#
# 功能:
#   - 系统内核参数优化（网络、内存、文件系统）
...
install_nodejs() {
    ...
    if curl --connect-timeout 10 --max-time 60 -fsSL https://deb.nodesource.com/setup_22.x -o "$node_setup" 2>/dev/null; then
        log_info "NodeSource setup 下载成功，执行安装..."
