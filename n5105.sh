#!/usr/bin/env bash
# VPS-youhua platform wrapper: Intel N5105/N5095 mini host

set -euo pipefail
: "${TERM:=xterm}"

PLATFORM_ID="n5105"
PLATFORM_NAME="N5105/N5095 x86_64 小主机"
PLATFORM_DESC="本地小主机或软路由设备，SSD 存储，普通保守优化"
VPSY_MEMORY_PROFILE="${VPSY_MEMORY_PROFILE:-medium}"
VPSY_CPU_PROFILE="${VPSY_CPU_PROFILE:-standard}"
VPSY_STORAGE_PROFILE="${VPSY_STORAGE_PROFILE:-ssd}"
VPSY_ROLE_PROFILE="${VPSY_ROLE_PROFILE:-production}"
VPSY_TUNING_PROFILE="${VPSY_TUNING_PROFILE:-local-host}"
VPSY_SYSCTL_FILE="${VPSY_SYSCTL_FILE:-/etc/sysctl.d/99-vps-youhua-n5105.conf}"
