#!/usr/bin/env bash
# =============================================================================
# N5105/N5095/x86 小主机 专用优化安装脚本
# 硬件: Intel N5105/N5095/J6412 x86_64, 4-16GB RAM
# 特点: 低功耗 x86, 适合长期运行, 稳定性优先
# =============================================================================
#
...
readonly PLATFORM_DESC="Intel N5105/N5095 x86_64, 低功耗小主机"

# N5105 优化参数
ZRAM_SIZE=0        # x86 通常 4GB+ 就够
[[ $SYS_MEM_MB -lt 4096 ]] && ZRAM_SIZE=512  # <4GB 启用 ZRAM
SWAPPINESS=20
TCP_BUF_MAX=25165824  # 24MB TCP 缓冲
...
detect_n5105() {
    ...
    local cpu_model
    cpu_model=$(cat /proc/cpuinfo | grep "model name" | head -1 | cut -d: -f2 | sed 's/^ //' || echo "")
    
    if echo "$cpu_model" | grep -qiE "N5105|N5095|N5095|J6412|J6413|Alder Lake|N100|N200"; then
        log_info "检测到 Intel x86 小主机: $cpu_model"
        PLATFORM_DESC="$cpu_model, 低功耗 x86_64"
        return 0
    fi
    
    # x86_64 架构检测
    if [[ "$(uname -m)" == "x86_64" ]]; then
        log_info "检测到 x86_64 平台 (可能是 N5105 或类似)"
        return 0
    fi
...
configure_sysctl_n5105() {
    log_step "配置 sysctl (N5105 优化)..."
    
    local sysctl_file="/etc/sysctl.d/99-openclaw.conf"
    backup_file "$sysctl_file"
    
    cat > "$sysctl_file" <<EOF
# N5105/N5095 小主机 OpenClaw 优化配置
# Intel x86_64 低功耗平台
...
optimize_network_n5105() {
    ...
    if [[ -n "$SYS_NET_IF" && -d "/sys/class/net/$SYS_NET_IF" ]]; then
        ...
        log_info "网卡 $SYS_NET_IF 优化完成 (Speed: $speed)"
        
        # RPS
        if [[ $SYS_CPU_CORES -gt 1 ]]; then
            local rps_file="/sys/class/net/${SYS_NET_IF}/queues/rx-0/rps_cpus"
            if [[ -f "$rps_file" ]]; then
                local mask
...
optimize_x86() {
    ...
    # 禁用节能
    for cpu in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do
        [[ -f "$cpu" ]] && cat "$cpu" | grep -q "powersave" && echo "ondemand" > "$cpu" 2>/dev/null || true
    done
...
main() {
    ...
    echo ""
    echo "═══════════════════════════════════════════════════════════════════════"
    echo -e "${GREEN}  ✅ N5105/N5095 小主机优化完成！${NC}"
    echo "═══════════════════════════════════════════════════════════════════════"
    echo ""
    echo -e "${CYAN}后续步骤:${NC}"
