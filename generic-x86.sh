...
detect_generic_x86() {
    ...
    if [[ "$(uname -m)" == "x86_64" ]]; then
        local cpu_model
        cpu_model=$(cat /proc/cpuinfo | grep "model name" | head -1 | cut -d: -f2 | sed 's/^ //' || echo "unknown")
        log_info "检测到 x86_64 平台: $cpu_model"
        PLATFORM_DESC="x86_64 VPS ($cpu_model)"
        return 0
    fi
...
EOF
    ...
    # 物理 Swap (如果内存 < 2GB 且磁盘充足)
    if [[ $SYS_MEM_MB -lt 2048 && $SYS_DISK_AVAIL_GB -gt 5 ]]; then
        local swap_mb=1024
        [[ $SYS_MEM_MB -le 512 ]] && swap_mb=512
...
configure_sysctl_generic() {
    log_step "配置 sysctl (通用 x86 优化)..."
    
    local sysctl_file="/etc/sysctl.d/99-openclaw.conf"
    backup_file "$sysctl_file"
    
    cat > "$sysctl_file" <<EOF
# 通用 x86_64 VPS OpenClaw 优化配置
# ${PROFILE_DESC}
...
vm.dirty_ratio = 15
vm.dirty_background_ratio = 5
vm.min_free_kbytes = ${MIN_FREE_KB:-32768}
vm.overcommit_memory = 1
...
main() {
    ...
    echo ""
    echo "═══════════════════════════════════════════════════════════════════════"
    echo -e "${GREEN}  ✅ 通用 x86_64 VPS 优化完成！${NC}"
    echo "═══════════════════════════════════════════════════════════════════════"
    echo ""
    echo -e "${CYAN}后续步骤:${NC}"
