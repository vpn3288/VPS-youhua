...
# ─────────────────────────────────────────────────────────────────────────────
# npm 缓存指向 tmpfs（TF卡保护，R4S 专项；其他平台通用）
# ─────────────────────────────────────────────────────────────────────────────
...
optimize_io_scheduler() {
    ...
    local is_ssd=false
    if [[ -b "$root_disk" ]]; then
        local dev_name; dev_name=$(basename "$root_disk" 2>/dev/null)
        if [[ -n "$dev_name" && -f "/sys/block/$dev_name/queue/rotational" ]] && \
           [[ "$(cat "/sys/block/$dev_name/queue/rotational" 2>/dev/null)" == "0" ]]; then
            is_ssd=true
        fi
    fi
...
EOF
    ...
    log_info "CPU governor: ${gov:-unknown}（已持久化）"
}
...
main() {
    ...
    echo "  - CPU governor 持久化（schedutil 三层自动）"
    echo "  - 防火墙（仅 lo 接口放行）"
    echo "  - /tmp tmpfs（减少磁盘写入）"
    echo "  - swap 优化（zram 保留，物理 swap 已清理）"
