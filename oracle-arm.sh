...
# =============================================================================
# 甲骨文云 ARM 专用优化安装脚本
# 硬件: Oracle Cloud ARM (Ampere Altra), 2核16GB, 100GB
# 特点: 云环境, 网络优化, 保留 oci-agent
# =============================================================================
#
...
install_openclaw() {
    ...
    if ! id -u "" &>/dev/null; then
        useradd -r -m -s /bin/bash -c "OpenClaw Service Account" "" 2>/dev/null || true
    fi
...
readonly PLATFORM_DESC="Oracle Cloud ARM (Ampere Altra), 2核16GB, 100GB"

# Oracle ARM 优化参数 - 16GB 内存充足
ZRAM_SIZE=0         # 16GB 足够，不需要 ZRAM
SWAPPINESS=10      # 低 swappiness
TCP_BUF_MAX=67108864   # 64MB TCP 缓冲 - 云环境需要更大缓冲
TCP_TW_BUCKETS=262144  # 更多 TW buckets
CT_MAX=262144      # 更大的连接追踪
INOTIFY_WATCHES=1048576

detect_oracle_arm() {
    log_step "检测 Oracle Cloud ARM..."
    
    # Oracle Cloud 检测方法
    if grep -qi "oracle" /etc/hostname 2>/dev/null; then
        log_info "检测到 Oracle Cloud (hostname)"
        return 0
    fi
    
    if dmidecode -s system-manufacturer 2>/dev/null | grep -qi "oracle"; then
        log_info "检测到 Oracle Cloud (dmidecode)"
        return 0
    fi
    
    if [[ -d /etc/oci ]]; then
        log_info "检测到 Oracle Cloud (/etc/oci 目录)"
        return 0
    fi
    
    # 检查 ARM 架构
    if [[ "$(uname -m)" == "aarch64" ]]; then
        log_warn "未明确检测到 Oracle Cloud，但架构为 ARM64"
        return 0
    fi
...
optimize_oracle_arm() {
    log_step "Oracle Cloud ARM 特定优化..."
    ...
    # Oracle Cloud 的网络接口通常是 en* 或 eth*
    # 检测并优化
    ...
    # Oracle Cloud 的 MTU 优化 (如果有 jumbo frame)
    for iface in /sys/class/net/en* /sys/class/net/eth*; do
        [[ -d "$iface" ]] || continue
        local name
        name=$(basename "$iface")
        if [[ -f "/sys/class/net/$name/mtu" ]]; then
            local mtu
            mtu=$(cat "/sys/class/net/$name/mtu" 2>/dev/null || echo "1500")
            if [[ "$mtu" -eq 9000 ]]; then
                log_info "网卡 $name 使用 Jumbo Frame (MTU: $mtu)"
            fi
        fi
    done
}
...
configure_sysctl_oracle_arm() {
    log_step "配置 sysctl (Oracle Cloud ARM 优化)..."
    
    local sysctl_file="/etc/sysctl.d/99-openclaw.conf"
    backup_file "$sysctl_file"
    
    cat > "$sysctl_file" <<EOF
# Oracle Cloud ARM OpenClaw 优化配置
# Ampere Altra, 2核16GB, 100GB

# 网络缓冲区 (Oracle Cloud 优化 - 64MB)
...
EOF
    ...
    sysctl -p "$sysctl_file" 2>/dev/null || true
    log_info "sysctl Oracle Cloud ARM 优化完成"
}
...
optimize_memory_oracle_arm() {
    ...
    log_info "内存优化完成 (16GB 充足)"
}

# Oracle Cloud ARM Docker 特殊安装
install_docker_oracle_arm() {
    ...
    log_step "安装 Docker (Oracle Cloud ARM)..."
...
main() {
    clear
    echo "═══════════════════════════════════════════════════════════════════════"
    echo -e "${GREEN}  甲骨文云 ARM 专用优化安装脚本 v${SCRIPT_VERSION}${NC}"
    echo "═══════════════════════════════════════════════════════════════════════"
    ...
    echo ""
    echo "═══════════════════════════════════════════════════════════════════════"
    echo -e "${GREEN}  ✅ Oracle Cloud ARM 优化完成！${NC}"
    echo "═══════════════════════════════════════════════════════════════════════"
    echo ""
    echo -e "${CYAN}后续步骤:${NC}"
    ...
    echo ""
    echo -e "${YELLOW}注意: Oracle Cloud 需要在控制台开放端口 18789${NC}"
    echo ""
}
