#!/usr/bin/env bash
# VPS-youhua platform wrapper: Oracle Cloud ARM

set -euo pipefail
: "${TERM:=xterm}"

PLATFORM_ID="oracle-arm"
PLATFORM_NAME="Oracle Cloud ARM"
PLATFORM_DESC="Oracle Cloud Ampere ARM 实例，普通保守优化"
VPSY_MEMORY_PROFILE="${VPSY_MEMORY_PROFILE:-large}"
VPSY_CPU_PROFILE="${VPSY_CPU_PROFILE:-cloud}"
VPSY_STORAGE_PROFILE="${VPSY_STORAGE_PROFILE:-cloud}"
VPSY_ROLE_PROFILE="${VPSY_ROLE_PROFILE:-production}"
VPSY_TUNING_PROFILE="${VPSY_TUNING_PROFILE:-arm-vps}"
VPSY_SYSCTL_FILE="${VPSY_SYSCTL_FILE:-/etc/sysctl.d/99-vps-youhua-oracle-arm.conf}"
