...
EOF
    ...
    log_info "ZRAM 已启用 (${ZRAM_SIZE}MB, zstd 算法)"
    ...
    log_info "内存架构优化完成 (ZRAM + ${SWAP_SIZE} Swap)"
}
...
optimize_io() {
    ...
    for disk in /sys/block/sd* /sys/block/vd* /sys/block/nvme*n*; do
        [[ ! -e "$disk" ]] && continue
        [[ $(basename "$disk") =~ nvme.*p ]] && continue  # 跳过分区
...
main() {
    ...
    echo "==========================================================="
    echo -e "${GREEN} VPS 完美优化脚本 v2.1 (代理专用 | 无BBR版) ${NC}"
    echo "==========================================================="
    echo ""
    echo -e "${BLUE}检测信息:${NC}"
    ...
    echo "  2. 重启后运行 ${CYAN}vps-status${NC} 查看状态"
    echo "  3. BBR 需手动配置 (未包含在此脚本中)"
    echo ""
    echo -e "${CYAN}输入 reboot 重启${NC}"
    echo ""
}
